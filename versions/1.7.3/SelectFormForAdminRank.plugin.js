...
function ShowFormModal({ rootProps }) {
  ...
  if (result) {
    ...
    return "Похоже, произошла ошибка..\nСообщите об этом разработчику: @sir.paws";
  }
}
  ...
  return BdApi.React.createElement(
    ...
    BdApi.React.createElement(
      ...
      (formatted.value && formatted.value.includes("{{dolj}}")) ? BdApi.React.createElement(
        Select,
        {
              ...
              `Ваша версия: \`${curr_version}\` | Новая версия: \`${latestVersion}\`\n\n\`СПИСОК ИЗМЕНЕНИЙ:\`\n\n${changelogs}\n\n> Подать баг: https://discord.gg/ZaNnGGNDWb`,
              {
                ...
                onConfirm: () => {
                  ...
                  request.get(
                    ...
                    (error, response, body) => {
                      if (error) {
                        BdApi.showToast(`Ошибка при загрузке: ${error}\nСообщите об этом разработчику: @sir.paws`, { type: "error" });
                        return false;
                      }
                      ...
                      if (response.statusCode == 200) {
                        ...
                        BdApi.showToast(`Ошибка при установке обновления.\nСообщите об этом разработчику: @sir.paws`, { type: "warning" });
                      }
                    }
                  );
                },
  ...
  request.get(
    ...
    (error, response, body) => {
      ...
      if (response.statusCode == 200) {
        ...
        const dolj = (result['dolj']) ? result['dolj'].data : 0;
        ...
        if (dolj.length > 0) {
          for (let i = 0; i < dolj.length; i++) {
            Dolj.push({
              "name": dolj[i]['name'],
              "disabled": dolj[i]['disabled'],
            });
          }
        }
        if (rank.length > 0) {
